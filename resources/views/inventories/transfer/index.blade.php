...
<div class="content-header">
    <div class="content-title">
        <h1>Transfer List</h1>
        <p>Manage inventory transfers</p>
    </div>
    <a href="{{ route('inventories.transfer.create') }}" class="btn-circle-add" title="Add New Transfer">
        +
    </a>
</div>
...
<div class="card datatable-wrapper">
    <div class="datatable-controls">
        ...
        <div class="datatable-search">
            <input type="text" id="dt-search" placeholder="Search transfer...">
        </div>
    </div>

    <div class="table-container">
        <table class="datatable" id="transfer-table">
            <thead>
                <tr>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const table = new DynamicDataTable('transfer-table', {
            url: "{{ route('inventories.transfer.data') }}",
            defaultSortCol: 'ID',
            defaultSortDir: 'desc',
            columns: [
                { 
                    ...
                    sortable: true,
                    render: (val, row) => {
                        const editUrl = "{{ route('inventories.transfer.edit', ':id') }}".replace(':id', row.ID);
        ...
        window.deleteRecord = (id) => {
            if (confirm('Are you sure you want to delete this transfer record?')) {
                fetch("{{ route('inventories.transfer.destroy', ':id') }}".replace(':id', id), {
                    method: 'DELETE',
                    headers: {
