...
<div class="content-header">
    <div class="content-title">
        <h1>Party List</h1>
        <p>Manage customers and client accounts</p>
    </div>
    <a href="{{ route('masters.party.create') }}" class="btn-circle-add" title="Add New Party">
        +
    </a>
</div>
...
<div class="card datatable-wrapper">
    <div class="datatable-controls">
        ...
        <div class="datatable-search">
            <input type="text" id="dt-search" placeholder="Search parties...">
        </div>
    </div>

    <div class="table-container">
        <table class="datatable" id="party-table">
            <thead>
                <tr>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const table = new DynamicDataTable('party-table', {
            url: "{{ route('masters.party.data') }}",
            defaultSortCol: 'ID',
            defaultSortDir: 'desc',
            columns: [
                { name: 'ID', sortable: true },
                { name: 'PartyName', sortable: true },
            ...
            actions: (row) => {
                const editUrl = "{{ route('masters.party.edit', ':id') }}".replace(':id', row.ID);
                return `
        ...
        window.deleteRecord = (id) => {
            if (confirm('Are you sure you want to delete this party?')) {
                fetch("{{ route('masters.party.destroy', ':id') }}".replace(':id', id), {
                    method: 'DELETE',
                    headers: {
