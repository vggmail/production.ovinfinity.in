...
@section('title', 'MRL Entry')

@section('content')
<div class="content-header">
    <div class="content-title">
        <h1>MRL Entry List</h1>
        <p>Manage Material Requisition Lists and dynamic item requisitions</p>
    </div>
    <a href="{{ route('store.mrlentry.create') }}" class="btn-circle-add" title="Add New MRL Entry">
        +
    </a>
</div>
...
<div class="card datatable-wrapper">
    <div class="datatable-controls">
        ...
        <div class="datatable-search">
            <input type="text" id="dt-search" placeholder="Search by ID or entry date...">
        </div>
    </div>

    <div class="table-container">
        <table class="datatable" id="mrlentry-table">
            <thead>
                <tr>
                    <th data-column="ID" style="width: 60px;">ID</th>
                    <th data-column="EntryDate">Entry Date</th>
                    <th data-column="TotalItems">Total Items</th>
                    <th data-column="TotalQuantity">Total Quantity</th>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const table = new DynamicDataTable('mrlentry-table', {
            url: "{{ route('store.mrlentry.data') }}",
            defaultSortCol: 'ID',
            defaultSortDir: 'desc',
            columns: [
                { 
                    ...
                    sortable: true,
                    render: (val, row) => {
                        const editUrl = "{{ route('store.mrlentry.edit', ':id') }}".replace(':id', row.ID);
        ...
        window.deleteRecord = (id) => {
            if (confirm('Are you sure you want to delete this MRL entry record?')) {
                fetch("{{ route('store.mrlentry.destroy', ':id') }}".replace(':id', id), {
                    method: 'DELETE',
                    headers: {
